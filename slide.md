...
# integration testならいけるかも？

なんかそれっぽいことが書いてある。

> 管理下にある依存とのコミュニケーションは実装の詳細である。一方、管理下にない依存とのコミュニケーションはテスト対象システムの観察可能な振る舞いの一部である。

> 統合テストの場合、管理下にある依存に対しては実際の依存を使い、管理下にない依存に対してはモックを使う。

（単体テストの考え方/使い方 p304）

> 統合テストは（中略）単体テストよりも優れた退行に対する保護が備わることになります。さらに統合テストはプロダクション・コードとの結びつきが単体テストよりも直接的でなくなるため、リファクタリングへの耐性も単体テストより備わることになります。

（単体テストの考え方/使い方 p264）

---

# integration testを書いてみる
...
- mockまみれ😣
  - 👉 mockするのは最下層のレイヤだけ。中間層はモックしない。
- repo.FindXXX/ListXXXの回数なんてチェックしたくない😣
  - 👉 integration testでは検証不要。
- ちょっとリファクタリングしただけで壊れる😣
  - 👉 仕様が変わらない限り、リファクタリングに対して頑健。
- DBとのやりとりの過程なんて知りたくない。知りたいのは最終的な状態😣
  - 👉 最終的なDBの状態だけ確認します。
